...
export default function TerminalPage() {
  ...
  const [platformData,     setPlatformData]     = useState<Record<string, Record<string, string | number>[]>>({})
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) { setLoading(false); setLoadingR(false); return }
  ...
  }).filter(p => p.id !== 'cash' || p.current > 0), [portfolio])
          ...
          <>
                          ...
                          {isLoading ? (
                            ...
                            data.map((pos, i) => {
                              ...
                              <div key={i} style={{ marginBottom: 10, paddingBottom: 10, borderBottom: i < data.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                  ...
                                  <span style={{ fontFamily: 'monospace', fontSize: 11, color: amt > 0 ? C.green : C.red }}>
                                    {amt > 0 ? 'LONG' : 'SHORT'}
                                  </span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                  <span style={{ fontFamily: 'monospace', fontSize: 10, color: C.dimText }}>
                                    PnL: <span style={{ color: pnl >= 0 ? C.green : C.red }}>${pnl.toFixed(2)}</span>
                                  </span>
                                  <span style={{ fontFamily: 'monospace', fontSize: 10, color: C.dimText }}>
                                    ${entry.toFixed(2)}
                                  </span>
                            ...
                            data.map((b, i) => (
                              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                ...
                                <span style={{ fontFamily: 'monospace', fontSize: 11, color: C.gold }}>{parseFloat(String(b.free)).toFixed(6)}</span>
                              </div>
                            ))
                          )}
                        </div>
            ...
            <div style={{ marginBottom: 1, marginTop: 1 }}>
              <div style={{ background: C.surface, padding: '12px 22px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <a href="/reportes" style={{ fontFamily: 'monospace', fontSize: 10, letterSpacing: '0.15em', color: C.dimText, textDecoration: 'none' }}>
                  VER PÁGINA COMPLETA →
                </a>
              </div>
              {loadingR ? (
                <div style={{ background: C.bg, padding: '28px', textAlign: 'center', fontFamily: 'monospace', fontSize: 12, color: C.muted }}>
                  Cargando reportes…
                </div>
              ) : reportes.length === 0 ? (
                <div style={{ background: C.bg, padding: '28px', textAlign: 'center', fontFamily: 'monospace', fontSize: 12, color: C.muted }}>
                  Aún no hay reportes publicados. Te avisaremos cuando se publique el primero.
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: C.border }}>
                  {reportes.map(r => (
                    <div key={r.id} style={{ background: C.bg, padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: "'Bebas Neue',Impact,sans-serif", fontSize: 28, color: C.gold, lineHeight: 1, minWidth: 44, flexShrink: 0 }}>
                        #{String(r.numero).padStart(3, '0')}
                      </span>
                      <div style={{ flex: 1, minWidth: 180 }}>
                        <div style={{ fontFamily: 'monospace', fontSize: 13, color: C.text, marginBottom: 3 }}>{r.titulo}</div>
                        <div style={{ fontFamily: 'monospace', fontSize: 11, color: C.dimText, marginBottom: 2 }}>{r.descripcion}</div>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                        {r.url_pdf ? (
                          ...
                          <span style={{ fontFamily: 'monospace', fontSize: 11, color: C.muted, border: `1px solid ${C.border}`, padding: '9px 18px' }}>
                            PRÓXIMAMENTE
                          </span>
                        )}
                      </div>
